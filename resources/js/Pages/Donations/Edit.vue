...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div v-if="donation.deleted_at"
            ...
            This donation has been deleted from the system, all records are for viewing only
        </div>
                ...
                <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                    <Accordion>
                        <template v-slot:title>
                            <span class="font-semibold text-lg my-2">Edit Donation</span>
                        </template>
                        <template v-slot:content>
                            <section>
                                    ...
                                    <div class="flex justify-between">
                                        <span>
                                            <p class="mt-1 text-sm text-gray-600">
                                                View or edit donation for the event.
                                            </p>
                                        </span>
                                        <span v-if="donation.deleted_at" class="mt-1 text-gray-600">
                                ...
                                <form @submit.prevent="form.patch(route('donations.update', donation))" class="mt-6 space-y-6">
                                    <div>
                                        <InputLabel for="name" value="Name" />

                                        <TextInput id="name" type="text" class="mt-1 block w-full" v-model="form.name"
                                            required :readonly="donation.deleted_at" autofocus autocomplete="name" />
                                    ...
                                    <div>
                                        ...
                                        <TextInput id="target" type="text" class="mt-1 block w-full" v-model="form.target"
                                            :readonly="donation.deleted_at" required />

                                        <InputError class="mt-2" :message="form.errors.target" />
                                    </div>

                                    <div class="flex items-center gap-4">
                                        <PrimaryButton v-if="!donation.deleted_at" :disabled="form.processing">Update
                                        </PrimaryButton>
